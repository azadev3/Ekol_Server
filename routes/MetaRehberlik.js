...
const MetaRehberlikModel = require('../models/MetaRehberlik');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-rehberlik', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaRehberlikModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-rehberlik', async (req, res) => {
  try {
    const datas = await MetaRehberlikModel.find();
...
router.put('/meta-tags-rehberlik/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaRehberlikModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaRehberlikModel not found' });
    }
    ...
    const updatedMetaTags = await MetaRehberlikModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-rehberlik-front', async (req, res) => {
  try {
