...
const MetaBloqlarModel = require('../models/MetaBloqlar');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-bloqlar', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaBloqlarModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-bloqlar', async (req, res) => {
  try {
    const datas = await MetaBloqlarModel.find();
...
router.put('/meta-tags-bloqlar/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaBloqlarModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaBloqlarModel not found' });
    }
    ...
    const updatedMetaTags = await MetaBloqlarModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-bloqlar-front', async (req, res) => {
  try {
