...
const MetaXeberlerModel = require('../models/MetaXeberler');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-xeberler', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaXeberlerModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-xeberler', async (req, res) => {
  try {
    const datas = await MetaXeberlerModel.find();
...
router.put('/meta-tags-xeberler/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaXeberlerModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaXeberlerModel not found' });
    }
    ...
    const updatedMetaTags = await MetaXeberlerModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-xeberler-front', async (req, res) => {
  try {
