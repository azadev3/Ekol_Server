...
const MetaSosialHeyatModel = require('../models/MetaSosialHeyat');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-sosialheyat', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaSosialHeyatModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-sosialheyat', async (req, res) => {
  try {
    const datas = await MetaSosialHeyatModel.find();
...
router.put('/meta-tags-sosialheyat/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaSosialHeyatModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaSosialHeyatModel not found' });
    }
    ...
    const updatedMetaTags = await MetaSosialHeyatModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-sosialheyat-front', async (req, res) => {
  try {
