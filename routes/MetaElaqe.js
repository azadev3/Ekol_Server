...
const MetaElaqeModel = require('../models/MetaElaqe');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-elaqe', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaElaqeModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-elaqe', async (req, res) => {
  try {
    const datas = await MetaElaqeModel.find();
...
router.put('/meta-tags-elaqe/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaElaqeModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaElaqeModel not found' });
    }
    ...
    const updatedMetaTags = await MetaElaqeModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-elaqe-front', async (req, res) => {
  try {
