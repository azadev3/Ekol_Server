...
const MetaXeberlerDaxiliModel = require('../models/MetaXeberlerDaxili');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-xeberlerdaxili', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaXeberlerDaxiliModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-xeberlerdaxili', async (req, res) => {
  try {
    const datas = await MetaXeberlerDaxiliModel.find();
...
router.put('/meta-tags-xeberlerdaxili/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaXeberlerDaxiliModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaXeberlerDaxiliModel not found' });
    }
    ...
    const updatedMetaTags = await MetaXeberlerDaxiliModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-xeberlerdaxili-front', async (req, res) => {
  try {
