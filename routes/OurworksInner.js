...
router.post(
 '/ourworksinner',
 checkUser,
 checkPermissions('create_gorduyumuzisler_daxili'),
 upload.none(),
 async (req, res) => {
  try {
...
router.delete(
 '/ourworksinner/:deleteid',
 checkUser,
 checkPermissions('delete_gorduyumuzisler_daxili'),
 async (req, res) => {
  try {
...
router.put('/ourworksinner/status/:id', async (req, res) => {
 try {
  ...
  const updatedPurch = await OurWorksInner.findByIdAndUpdate(id, { statusActive: statusActive }, { new: true })
   .lean()
   .exec();
