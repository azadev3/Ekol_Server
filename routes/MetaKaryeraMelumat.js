...
const MetaKaryeraMelumatModel = require('../models/MetaKaryeraMelumat');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-karyeraimkanlariumumimelumat', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaKaryeraMelumatModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-karyeraimkanlariumumimelumat', async (req, res) => {
  try {
    const datas = await MetaKaryeraMelumatModel.find();
...
router.put('/meta-tags-karyeraimkanlariumumimelumat/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaKaryeraMelumatModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaKaryeraMelumatModel not found' });
    }
    ...
    const updatedMetaTags = await MetaKaryeraMelumatModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-karyeraimkanlariumumimelumat-front', async (req, res) => {
  try {
