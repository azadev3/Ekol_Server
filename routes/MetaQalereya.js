...
const MetaQalereyaModel = require('../models/MetaQalereya');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-qalereya', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaQalereyaModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-qalereya', async (req, res) => {
  try {
    const datas = await MetaQalereyaModel.find();
...
router.put('/meta-tags-qalereya/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaQalereyaModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaQalereyaModel not found' });
    }
    ...
    const updatedMetaTags = await MetaQalereyaModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-qalereya-front', async (req, res) => {
  try {
