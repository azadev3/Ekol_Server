...
const MetaAvadanliqlarModel = require('../models/MetaAvadanliqlar');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-avadanliqlar', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaAvadanliqlarModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-avadanliqlar', async (req, res) => {
  try {
    const datas = await MetaAvadanliqlarModel.find();
...
router.put('/meta-tags-avadanliqlar/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaAvadanliqlarModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaAvadanliqlarModel not found' });
    }
    ...
    const updatedMetaTags = await MetaAvadanliqlarModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-avadanliqlar-front', async (req, res) => {
  try {
