...
const MetaSatinalmaQaydalariModel = require('../models/MetaSatinalmaQaydalari');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-satinalmaqaydalari', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaSatinalmaQaydalariModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-satinalmaqaydalari', async (req, res) => {
  try {
    const datas = await MetaSatinalmaQaydalariModel.find();
...
router.put('/meta-tags-satinalmaqaydalari/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaSatinalmaQaydalariModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaSatinalmaQaydalariModel not found' });
    }
    ...
    const updatedMetaTags = await MetaSatinalmaQaydalariModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-satinalmaqaydalari-front', async (req, res) => {
  try {
