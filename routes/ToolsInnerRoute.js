...
router.post(
 '/toolsinner',
 checkUser,
 checkPermissions('create_avadanliqlar_daxili'),
 upload.none(),
 async (req, res) => {
  try {
...
router.put(
 '/toolsinner/:editid',
 checkUser,
 checkPermissions('update_avadanliqlar_daxili'),
 upload.none(),
 async (req, res) => {
  try {
   const { editid } = req.params;
...
router.put('/toolsinner/status/:id', async (req, res) => {
 try {
  const { id } = req.params;
  ...
  const updatedPurch = await ToolsInnerModel.findByIdAndUpdate(id, { statusActive: statusActive }, { new: true })
   .lean()
   .exec();
