...
router.put(
  '/servicespage/:editid',
  checkUser,
  checkPermission('update_xidmetler_daxili'),
  uploadConfig.single('imgback'),
  async (req, res) => {
    try {
      const { editid } = req.params;
      const { title_az, title_en, title_ru, description_az, description_en, description_ru, slogan_az, slogan_en, slogan_ru } = req.body;
      ...
      const updatedServicePage = await ServicesPage.findByIdAndUpdate(editid, { $set: updatedData }, { new: true }).lean().exec();
