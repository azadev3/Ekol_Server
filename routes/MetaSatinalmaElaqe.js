...
const MetaSatinalmaElaqeModel = require('../models/MetaSatinalmaElaqe');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-satinalmaelaqe', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaSatinalmaElaqeModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-satinalmaelaqe', async (req, res) => {
  try {
    const datas = await MetaSatinalmaElaqeModel.find();
...
router.put('/meta-tags-satinalmaelaqe/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaSatinalmaElaqeModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaSatinalmaElaqeModel not found' });
    }
    ...
    const updatedMetaTags = await MetaSatinalmaElaqeModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-satinalmaelaqe-front', async (req, res) => {
  try {
