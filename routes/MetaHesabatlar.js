...
const MetaHesabatlarModel = require('../models/MetaHesabatlar');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-hesabatlar', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaHesabatlarModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-hesabatlar', async (req, res) => {
  try {
    const datas = await MetaHesabatlarModel.find();
...
router.put('/meta-tags-hesabatlar/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaHesabatlarModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaHesabatlarModel not found' });
    }
    ...
    const updatedMetaTags = await MetaHesabatlarModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-hesabatlar-front', async (req, res) => {
  try {
