...
const MetaGorduyumuzIslerModel = require('../models/MetaGorduyumuzIsler');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-gorduyumuzisler', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaGorduyumuzIslerModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-gorduyumuzisler', async (req, res) => {
  try {
    const datas = await MetaGorduyumuzIslerModel.find();
...
router.put('/meta-tags-gorduyumuzisler/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaGorduyumuzIslerModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaGorduyumuzIslerModel not found' });
    }
    ...
    const updatedMetaTags = await MetaGorduyumuzIslerModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-gorduyumuzisler-front', async (req, res) => {
  try {
