...
const MetaBloqlarDaxili = require('../models/MetaBloqlarDaxili');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-bloqlardaxili', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaBloqlarDaxili.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-bloqlardaxili', async (req, res) => {
  try {
    const datas = await MetaBloqlarDaxili.find();
...
router.put('/meta-tags-bloqlardaxili/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaBloqlarDaxili.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaBloqlarDaxili not found' });
    }
    ...
    const updatedMetaTags = await MetaBloqlarDaxili.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-bloqlardaxili-front', async (req, res) => {
  try {
