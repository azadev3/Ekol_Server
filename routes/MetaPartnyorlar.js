...
const MetaPartnyorlarModel = require('../models/MetaPartnyorlar');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-partnyorlar', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaPartnyorlarModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-partnyorlar', async (req, res) => {
  try {
    const datas = await MetaPartnyorlarModel.find();
...
router.put('/meta-tags-partnyorlar/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaPartnyorlarModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaPartnyorlarModel not found' });
    }
    ...
    const updatedMetaTags = await MetaPartnyorlarModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-partnyorlar-front', async (req, res) => {
  try {
