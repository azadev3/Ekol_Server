...
router.put(
 '/lisansepage/:editid',
 checkUser,
 checkPermissions('update_lisenziyalar'),
 upload.none(),
 async (req, res) => {
  try {
   const { editid } = req.params;
...
router.put('/lisansepage/status/:id', async (req, res) => {
 try {
  const { id } = req.params;
  ...
  const updatedPurch = await LisansePage.findByIdAndUpdate(id, { statusActive: statusActive }, { new: true })
   .lean()
   .exec();
