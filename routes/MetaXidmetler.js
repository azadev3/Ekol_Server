...
const MetaXidmetlerModel = require('../models/MetaXidmetler');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-xidmetler', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaXidmetlerModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-xidmetler', async (req, res) => {
  try {
    const datas = await MetaXidmetlerModel.find();
...
router.put('/meta-tags-xidmetler/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaXidmetlerModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaXidmetlerModel not found' });
    }
    ...
    const updatedMetaTags = await MetaXidmetlerModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-xidmetler-front', async (req, res) => {
  try {
