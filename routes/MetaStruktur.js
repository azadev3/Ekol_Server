...
const MetaStrukturModel = require('../models/MetaStruktur');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-struktur', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaStrukturModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-struktur', async (req, res) => {
  try {
    const datas = await MetaStrukturModel.find();
...
router.put('/meta-tags-struktur/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaStrukturModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaStrukturModel not found' });
    }
    ...
    const updatedMetaTags = await MetaStrukturModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-struktur-front', async (req, res) => {
  try {
