...
const MetaSertifikatlarModel = require('../models/MetaSertifikatlar');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-sertifikatlar', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaSertifikatlarModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-sertifikatlar', async (req, res) => {
  try {
    const datas = await MetaSertifikatlarModel.find();
...
router.put('/meta-tags-sertifikatlar/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaSertifikatlarModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaSertifikatlarModel not found' });
    }
    ...
    const updatedMetaTags = await MetaSertifikatlarModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-sertifikatlar-front', async (req, res) => {
  try {
