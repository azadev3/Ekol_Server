...
const MetaSatinalmaModel = require('../models/MetaSatinalma');
const upload = require('../config/MulterConfig');

router.post('/meta-tags-satinalmaelanlari', upload.none(), async (req, res) => {
  try {
    const updateData = {
      meta_title: {
    ...
    const savedData = await MetaSatinalmaModel.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
router.get('/meta-tags-satinalmaelanlari', async (req, res) => {
  try {
    const datas = await MetaSatinalmaModel.find();
...
router.put('/meta-tags-satinalmaelanlari/:editid', upload.none(), async (req, res) => {
  try {
    ...
    const existingHero = await MetaSatinalmaModel.findById(editid).exec();

    if (!existingHero) {
      return res.status(404).json({ error: 'MetaSatinalmaModel not found' });
    }
    ...
    const updatedMetaTags = await MetaSatinalmaModel.findByIdAndUpdate(editid, { $set: updatedMetaData }, { new: true }).lean().exec();
...
// for front
router.get('/meta-tags-satinalmaelanlari-front', async (req, res) => {
  try {
